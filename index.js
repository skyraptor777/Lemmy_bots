...
const lemmy_bot_1 = __importDefault(require("lemmy-bot"));
...
let client = new lemmy_js_client_1.LemmyHttp(baseURL);
/**
...
*/
const lemmywinx = new lemmy_bot_1.default({
    ...
    handlers: {
         ...
         */
        mention: ({ mentionView: { comment: { creator_id, id: comment_id, content }, post: { id } }, botActions: { createComment, voteComment, createPost } }) => {
            createComment({
...
lemmywinx.start();
//
async function js_client_login(community_name) {
    let form = {
    ...
    let community_id_form = { name: community_name };
    ...
    */
    const response = await login_response.then((value) => {
        jwt = value.jwt;
...
function update_fixture_table() {
    const new_md_output = new Promise((resolve, reject) => {
        resolve((0, fixtures_1.get_list_of_fixtures)());
    });
    new_md_output.then(value => {
        let edit_community_form = {
            auth: jwt,
            community_id: 9451,
            description: value
        };
